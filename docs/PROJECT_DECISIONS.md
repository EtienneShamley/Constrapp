...
> **ADR numbering note.** **ADR-28** is reserved for the parked Documents &
> Drawings branch and **ADR-29** for the parked Project Timeline branch. Neither
> is merged to `main` yet, so those numbers are held rather than reused, and no
> placeholder entry is created for them.
