...
import { isPriced, formatQuantity } from '../../../lib/boq'

// ── Void a BOQ item ──────────────────────────────────────────────────────────
//
// Void is terminal and requires a non-whitespace reason — rules-enforced. The
// item is never deleted: it is retained BOQ history (and remains currency-lock
// evidence when priced), contributing nothing to any total or comparison.

export default function BoqItemVoidModal({ item, currencyCode, onConfirm, onClose }) {
  ...
  async function submit(e) {
    e.preventDefault()
    if (!reason.trim()) { setError('Enter a reason for voiding this BOQ item.'); return }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-[480px] bg-brand-surface border border-brand-border rounded-xl shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-brand-border">
          <h2 className="text-[15px] font-bold text-brand-text m-0">Void BOQ item</h2>
          <button
            onClick={onClose}
            aria-label="Close"
        ...
        <form onSubmit={submit} className="p-5">
          <p className="m-0 text-[12.5px] text-brand-text">
            {item.itemNumber ? `${item.itemNumber} · ` : ''}{item.description} ·{' '}
            {formatQuantity(item.quantity)} {item.unit} ·{' '}
            <span className="font-semibold">
              {isPriced(item.rate) ? formatCurrency(item.amount, currencyCode, { precise: true }) : 'Unpriced'}
            </span>
          </p>
          <p className="m-0 mt-1.5 text-[11.5px] text-brand-muted">
            Voiding is permanent. The item is kept as BOQ history and stops counting toward the BOQ total
            and the budget comparison. Nothing else changes — the BOQ never feeds a financial figure.
          </p>
          ...
          <input
            ...
            placeholder="e.g. Superseded by revised takeoff"
            value={reason}
          ...
          <div className="flex justify-end gap-2 mt-5 pt-4 border-t border-brand-border">
            ...
            <Btn variant="danger" type="submit" disabled={saving}>{saving ? 'Voiding…' : 'Void item'}</Btn>
          </div>
        </form>
      </div>
    </div>
