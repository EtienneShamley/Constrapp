...
- **Variations** — *Placeholder.* Approved scope changes that update budget and commitment and flow into claims and forecast — the connector between commitment, claims, invoices, and forecast. *Planned* (activates the reserved `variationId`).
- **Forecast & Cash Flow** — *Placeholder.* Forecast cost to complete, cash-flow curves, and project margin — the outputs that close the project-control loop. *Planned.*
