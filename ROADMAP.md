...
**7. Credit Notes** — *Retention Release shipped (see Completed Foundations).*
Supplier credits (`docType`/`adjustsInvoiceId`) are the remaining piece. Supplier
...
**Retention Release shipped (ADR-30)**: the payable basis is now the stored
`payableTotal` **plus** posted retention released, so retained money finally has
a route to becoming payable — via an ordinary Supplier Payment, with no change to
the payment or allocation model. **Remaining (deferred):** client retention,
