...
- **Cash Flow** — *Placeholder.* Time-phased cash-in/cash-out curves and peak funding — the remaining output that closes the project-control loop. Client Invoices / Accounts Receivable now exist; **Payments and Receipts remain the outstanding prerequisite**, without which no cash figure would be honest. *Planned* (after Payments and Receipts).
...
- **Payments and Receipts** — *Future.* **Client receipts** allocated against issued client invoices — the missing half of Accounts Receivable, and the reason no invoice may yet be described as paid — plus payments against posted supplier invoices (the reserved `paid` status / `paidAt`) and retention release. Balances will be derived at read time from receipt records. Accounting integrations (Xero, MYOB, QuickBooks) attach via the `externalRefs` fields on POs, claims, and invoices.
