...
frontend/
  ...
  src/
    ...
    lib/            firebase.js, formatters.js, currency.js, nav.js, projectTabs.js,
                    ...
                    aggregation), contacts.js (pure domain logic)
...
- **Cash Flow is a read-only consumer of cash records — actual only, gross only.** The Actual Cash Flow view derives everything at read time from posted Client Receipts and posted Supplier Payments (`lib/cashFlow.js` over `cashInRows()`/`cashOutRows()`): it consumes the **total transaction `amount`** (never `allocatedTotal` — an unallocated advance is still cash that moved) on the **transaction date** (`receiptDate`/`paymentDate`, never `createdAt`/`postedAt`), groups by `date.slice(0, 7)`, and **stores nothing and writes nothing** — no receipt, payment, invoice, PO, claim, variation, forecast line, budget line, or baseline is ever mutated by it. The cumulative position starts at **zero** and is net project cash movement, **never presented as a bank balance** (no bank account, opening balance, financing, or GST/BAS remittance is modelled). Cash figures are **gross** and are never added to an ex-GST budget, forecast, or margin figure. Forecast Cash Flow (due-date collections, manual timing, peak funding) is a separate, later foundation
