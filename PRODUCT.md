...
- **Cash Flow** — *Placeholder.* Time-phased cash-in/cash-out curves and peak funding — the remaining output that closes the project-control loop. **Actual Cash In now exists** (posted Client Receipts carry an amount, a receipt date, a client, and the project currency). **Actual Cash Out does not** — Supplier Payments are the outstanding prerequisite, and until they ship any cash-out figure would be forecast presented as fact. *Planned* (after Supplier Payments).
...
- **Payments and Receipts** — *Partial.* **Client Receipts are implemented** (see Tier 1 above): cash received from clients, allocated against issued client invoices, with every balance derived at read time. **Supplier Payments (money out) are the next branch** and reuse the same shared allocation foundation; until they exist there is no actual Cash Out, and the supplier-invoice `paid` status / `paidAt` field remain **reserved and unused** — they are approved for *deprecation in place*, not activation, so payment state never has two sources of truth. Retention release remains unmodelled. Accounting integrations (Xero, MYOB, QuickBooks) attach via the `externalRefs` fields on POs, claims, invoices, and receipts.
