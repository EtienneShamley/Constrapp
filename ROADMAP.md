...
- [x] **Documents & Drawings (foundation)** — the first module that stores **files**, and therefore the branch that introduces Constrapp's **second trust boundary**: `frontend/storage.rules`. The **Documents** project tab becomes a nested workspace (`ProjectDocumentsLayout`, modelled on Commercial): a **Drawings** register (index), a **drawing detail ROUTE** (`documents/drawings/:drawingId` — a drawing is linked to, shared and bookmarked, so it is never a modal), and a **General Documents** register (`documents/general`). No new top-level project tab. **Drawing model = master + immutable revisions.** A master (`…/projects/{projectId}/drawings/{drawingId}`, random id) is **BORN EMPTY** — `currentRevisionId: null`, `revisionCount: 0` — and only starts advertising a revision once bytes are genuinely stored, so a failed upload can never leave a drawing claiming a revision that is not there (rules enforce the empty birth). Each revision (`…/drawings/{drawingId}/revisions/{revisionId}`) is created **current** by ONE Firestore transaction that also supersedes the previous current revision and moves the master (`currentRevisionId`, `currentRevisionCode`, `currentRevisionIssuedDate`, `revisionCount` **+1 exactly**, rules-enforced). **⚠️ Concurrency ABORTS, never resolves:** the transaction re-reads the master and compares `expectedCurrentRevisionId` against the pointer the UI was showing; if another user issued in the meantime it throws *"Another user issued a revision while you were uploading. Review the drawing before re-issuing."* and promotes nothing — auto-promoting would silently supersede unseen work. **Revisions are ordered by the integer `revisionSequence`, never by `revisionCode`** ("A", "B", "P1", "C2", "10" do not sort lexically into issue order). **Lifecycle** `current → superseded` / `current → withdrawn` / `superseded → current` / `superseded → withdrawn`, withdrawn **terminal**; masters `active → withdrawn`, terminal. **Withdrawing the current revision forces an explicit succession decision** — the user either nominates an earlier non-withdrawn revision to reinstate, or states there is no replacement (which withdraws the master and leaves it with no current revision). Nothing is auto-promoted; **nothing is ever hard-deleted anywhere**. Opening a non-current revision shows a **non-dismissible warning in words** (*SUPERSEDED — Revision B / Do not build from this drawing. Current revision is C.*, *WITHDRAWN / Do not use this drawing.*) — **never colour alone**. **General documents** (`…/projects/{projectId}/documents/{documentId}`) are a **flat** register — ten categories, **no folders**, **no revision subcollection**: a replacement is a NEW record and the old one becomes `superseded` with a `supersededByDocumentId` forward link, both files preserved, committed in one transaction. Visibility is `project` (every provisioned company member) or `internal` (company_admin/project_manager/qs), shown with a lock glyph **and the word "Internal"**. **⚠️ Rules are not filters:** a non-internal role subscribes with `where('visibility','==','project')` because one internal document would otherwise fail the WHOLE query — the filter makes the allowed query answerable; the rule is the control. **Files:** PDF/PNG/JPEG only, 50 MB drawings / 25 MB documents, zero-byte rejected. Bytes go to **Firebase Storage**, metadata to Firestore, **Storage FIRST, Firestore SECOND** — an orphaned object beats a register row pointing at bytes that never arrived, and orphans are **accepted and documented** because a delete permission able to tidy them would also let a client destroy an issued revision. Paths are deterministic and company-namespaced (`companies/{c}/projects/{p}/drawings/{d}/{r}/original.{ext}`); **the uploaded filename is never object identity**, only metadata, and **`getDownloadURL()` output is NEVER persisted** — URLs are minted on demand and discarded. Uploads use `uploadBytesResumable` with progress, cancel and an honest retry (a retry mints a **new** id, because objects are create-only). **Roles:** drawing READS are open to **every provisioned company member including subcontractor and client** — the first collection in the app with a membership-only read, because withholding the current sheet is the real safety risk; drawing WRITES are company_admin/project_manager (**QS excluded** in this branch); general-document writes are company_admin/project_manager/qs. **Storage Rules enforce** tenant/path isolation, membership, writer role, allowed content type, the exact `original.{ext}` object name, the size ceiling, non-zero size, **create-only semantics (no overwrite, no update, no delete)**, a `firestore.get()` visibility check for documents, and a catch-all deny. **Firestore Rules enforce** the born-empty master, the four `hasOnly`-restricted master update shapes, the +1 revision count, revision immutability (file **and** authored identity), an **exact `storagePath` recomputed from the path segments** — so a revision can never point at another tenant's bytes — legal transitions only, non-whitespace withdrawal reasons, and `delete: if false` everywhere. **Test infrastructure change:** `firebase.json` gains a `storage` rules pointer and a storage emulator (port 9199) and `npm run test:rules` now starts `firestore,storage` — **one command, both boundaries**, no new dependency. Rules **207 → 390** (Firestore 344, Storage 46); unit **173 → 301**. **Reserved, never fabricated:** `pageCount` stays `null` and `sheetSize` stays `''` (rules reject any other value) — the immutable **`{ drawingId, revisionId }`** pair is the identity a future takeoff will reference, never `drawingNumber`, never `currentRevisionId`, never the filename. **⚠️ Not enforced (client/business-only):** drawingNumber and revisionCode uniqueness, exactly-one-current-revision against a direct SDK sibling write, `currentRevisionId` existence (it is created in the same transaction), real file bytes behind the declared type/size, **project-specific membership** (membership is company-wide), and AV scanning. **⚠️ Not included:** photos, DWG, markup/annotation, OCR, AI, takeoff, quantities, BOQ, tendering, drawing comparison, thumbnails, AV-scanning backend, offline sync. **No financial collection, hook, page or rules block was touched.** No migration. See ADR-28

Firestore security rules for all of the above are written in `frontend/firestore.rules`, and Cloud Storage rules in `frontend/storage.rules`; both are published manually.
...
Drawings (drawing measurement → BOQ quantity) — **the document-control foundation has landed** (see Completed Foundations); what remains is the measurement half, which depends on the BOQ. Site Photos (→ progress/claim evidence), Timeline (delay → forecast impact). Each lands only with its commercial input/output defined.
