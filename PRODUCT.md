...
- **Cash Flow** — *Placeholder — now unblocked.* Time-phased cash-in/cash-out curves and peak funding: the remaining output that closes the project-control loop. **Both actual directions now exist** — Cash In from posted Client Receipts and Cash Out from posted Supplier Payments, each carrying an amount, a transaction date, a counterparty, and the project currency. Cash Out is always the **total** payment amount (an unallocated supplier advance is still cash that left the bank), consumed on `paymentDate`/`receiptDate`, never on a created or posted timestamp, and never summed across currencies. *Planned — no cash-flow route, chart, period, or aggregation is built.*
