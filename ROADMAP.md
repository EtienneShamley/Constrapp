...
- **3c-ii. Forecast Cash Flow** — *shipped (see Completed Foundations).*
  ...
  untimed-suppression.
- **3c-iii. Cash Flow visualisation** — *next.* Charts (Recharts is already a
  dependency) and optional date-range filtering. **Invoice retiming** — letting
  a past-due invoice balance be manually moved to a future month via reserved
...
Flow shipped → Cash Flow visualisation next.**
